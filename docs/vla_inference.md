...
界面可编辑任务文本，并可在运行期间开始或停止双相机 MP4 录制。`Apply Task`
会立即请求新文本对应的策略结果，并安全丢弃尚未执行的旧动作队列。`Apply Mode` 才会安全停止当前
伺服会话并按新的同步/异步模式重新连接。`Restore Initial State + Apply Task`
会停止当前 VLA 会话、插值恢复 JSON 初始态，然后按当前任务文本重新开始推理。

界面布局中，`Apply Task` 位于任务文本框右侧，`Apply Mode` 位于推理模式选择框右侧。
初始态恢复按钮只在已启动会话后可用；它会停止旧会话，因此不要在夹爪接触物体或机器人
